...
int main() {
    ...
    G_build();
    calc_ans(1);
    for (int id = fir_q; id; id = nex[id]) {
        int u = 1, succ = true;
        for (int i = 0; i < str[id].size(); i++) {
            int c = str[id][i] - 'a';
            if (nd[u].ch[c]) u = nd[u].ch[c];
            else { succ = false; break; }
        }
        if (!succ) { printf("0\n"); continue; }
        while (nex[t1] && nex[t1] < id) t1 = nex[t1];
        while (nex[t2] && nex[t2] < id) t2 = nex[t2];

        if (t1 < id && t2 < id) 
            printf("%d\n", ST_query(tid[u], 1, str_len, p[t2]+str[id].size()-1, p[t1]+str[t1].size()-2));
        else if (t1 < id) 
            printf("%d\n", ST_query(tid[u], 1, str_len, len2+str[id].size(), p[t1]+str[t1].size()-2));
        else if (t2 < id)
            printf("%d\n", ST_query(tid[u], 1, str_len, p[t2]+str[id].size()-1, len2));
    }
    return 0;
}